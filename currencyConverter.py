...
# Converts between ZWL (ZWL) and USD ($)
...
class CurrencyConverter:
    ...
    def __init__(self, master):
        ...
        # Variables for toggling between ZWL and USD
        self.dateText = StringVar()
        self.fromCurrency = StringVar()
        self.fromCurrency.set("ZWL")
        ...
        eurosButton = Radiobutton(radioFrame, text = "ZWL -> $", variable = self.cur, value = 18, command = self.locale)
        eurosButton.grid(row = 1, column = 0)

        poundsButton = Radiobutton(radioFrame, text = "$ -> ZWL", variable = self.cur, value = 1, command = self.locale)
